...
$(document).ready(function () {
    ...
    //     }
    //     else {
    //         // showing users an error message for an unsuccessful conversion
    //         $(".apiCall-btctoNaira").text(`There was an error processing your request ${converter.message}`);
    //         $(".apiCall-ethToNaira").text(`There was an error processing your request ${converter.message}`);
    //         $(".apiCall-BnctoNaira").text(`There was an error processing your request ${converter.message}`);
    //         $(".apiCall-tethToNaira").text(`There was an error processing your request ${converter.message}`);
    //         $(".apiCall-rippletoNaira").text(`There was an error processing your request ${converter.message}`);
    //     }
    // }

    // working with the class for converting currencies
    // CurrencyConvertService.convertCurrencytToNaira()

    // calling the convertCurrecy function in a .then() for handling a resolved or rejected promise
    // .then( function(gottenConversionResponse){
    //     convertCurrency(gottenConversionResponse);
    // })
    ...
    function getCryptos(myCoinRequest) {
        ...
        if (myCoinRequest[0]) {
            ...
            // showing users the message property if I get an error
            $(".content-name").text(`There was an error processing your request ${myCoinRequest.message}`);
            $(".content-id").text(`There was an error processing your request ${myCoinRequest.message}`);
            $(".content-marketcapRank").text(`There was an error processing your request ${myCoinRequest.message}`);
            $(".content-symbol").text(`There was an error processing your request ${myCoinRequest.message}`);

        }
    }

    // making use of the class in business logic.... No parameters needed for this call
    CryptoService.getAllCoins()
        // no semicolons here aso because we're chaining promises to each other

        // calling the getCryptos function in a .then() for handling a resolved or rejected promise
        .then(function (gottenCryptos) {
            return getCryptos(gottenCryptos);
        });
    ...
    $("form.searching").submit(function (event) {
        ...
        function getSearchedCrypto(mySearchResponse) {
            if (mySearchResponse.coins[0]) {
                ...
                $(".error").text(`Sorry, there was an error in processing Your Request ${mySearchResponse.message}`);
               
            }
        }

        SearchFeature.searchMeCrypto(userSearch)

        .then( function(searchResponseGotten){
            getSearchedCrypto(searchResponseGotten);
        })
